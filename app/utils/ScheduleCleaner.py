...
def count_days_between(start_date: str, end_date: str) -> int:
    if start_date == end_date:
        ...
        logger.info(
            f"Расстояние между {start_date} и {end_date} дней: {(end - start).days}"
        )
...
def insert_data(dictionary: Dict[str, Dict[str, dict]], month: int) -> None:
    ...
    for dayDate in dictionary.keys():
        ...
        if (
            ...
            and dictionary[dayDate]["День Недели"] != "суббота"
            and count_days_between(start_date, dayDate) >= 2
        ):
            ...
            for day in current_week.keys():
                if current_week[day] < maximum_week:
                    logger.info(
                        f"была {current_week[day]} неделя - стала {maximum_week}"
                    )
        ...
        for subjectName in day["Предметы"].keys():
            ...
            subjectTimePosition = Subjects_Time_positions[subjectCount][weekCount][
                dayOfWeekNum
            ]
...
def do_dict():
    ...
    )  # Не переносить вывод на новую строку
